...
static bson_t *role_bson_create (const Role *role) {
	...
	if (doc) {
		...
		// add empty actions array
		bson_t actions_array = BSON_INITIALIZER;
		bson_append_array (doc, "actions", -1, &actions_array);
	}
...
unsigned int role_save (const Role *role) {

	return mongo_insert_one (
		roles_model, role_bson_create (role)
	);

}

static bson_t *role_update_bson_create (const Role *role) {
	...
	if (doc) {
		...
		if (role->n_actions) {
			char buffer[BSON_ARRAY_BUFFER_SIZE] = { 0 };
			const char *key = NULL;
			size_t keylen = 0;

			for (unsigned int idx = 0; idx < role->n_actions; idx++) {
				keylen = bson_uint32_to_string (idx, &key, buffer, BSON_ARRAY_BUFFER_SIZE);
				(void) bson_append_utf8 (
					&actions_array, key, (int) keylen, role->actions[idx], -1
				);
			}
		}
...
unsigned int role_update (const Role *role) {

	return mongo_update_one (
		roles_model,
		role_query_by_oid (&role->oid),
		role_update_bson_create (role)
	);
